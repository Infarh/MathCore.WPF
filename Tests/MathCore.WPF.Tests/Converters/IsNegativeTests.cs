...
/// <summary>Тесты для конвертера проверки на отрицательность IsNegative</summary>
[TestClass]
public class IsNegativeTests
{
    /// <summary>Проверка отрицательного числа возвращает true</summary>
    [TestMethod]
    public void Convert_NegativeNumber_ReturnsTrue()
    {
        ...
        Assert.IsTrue((bool)result!, "Отрицательное число должно возвращать true");
    }

    /// <summary>Проверка положительного числа возвращает false</summary>
    [TestMethod]
    public void Convert_PositiveNumber_ReturnsFalse()
    {
        ...
        Assert.IsFalse((bool)result!, "Положительное число должно возвращать false");
    }
    ...
    /// <summary>Проверка отрицательной бесконечности возвращает true</summary>
    [TestMethod]
    public void Convert_NegativeInfinity_ReturnsTrue()
    {
        ...
        Assert.IsTrue((bool)result!, "Отрицательная бесконечность должна возвращать true");
    }

    /// <summary>Проверка положительной бесконечности возвращает false</summary>
    [TestMethod]
    public void Convert_PositiveInfinity_ReturnsFalse()
    {
        ...
        Assert.IsFalse((bool)result!, "Положительная бесконечность должна возвращать false");
    }
    ...
    [DataRow(-100.0)]
    [DataRow(-1e10)]
    public void Convert_VariousNegativeValues_ReturnsTrue(double value)
    {
        IValueConverter converter = new IsNegative();
        var result = converter.Convert(value, typeof(bool), null, CultureInfo.InvariantCulture);
        
        Assert.IsTrue((bool)result!, $"Отрицательное значение {value} должно возвращать true");
    }
    ...
    [DataRow(100.0)]
    [DataRow(1e10)]
    public void Convert_VariousPositiveValues_ReturnsFalse(double value)
    {
        IValueConverter converter = new IsNegative();
        var result = converter.Convert(value, typeof(bool), null, CultureInfo.InvariantCulture);
        
        Assert.IsFalse((bool)result!, $"Положительное значение {value} должно возвращать false");
    }

    /// <summary>Проверка очень малого положительного числа (Epsilon)</summary>
    [TestMethod]
    public void Convert_Epsilon_ReturnsFalse()
    {
        IValueConverter converter = new IsNegative();
        var result = converter.Convert(double.Epsilon, typeof(bool), null, CultureInfo.InvariantCulture);
        
        Assert.IsFalse((bool)result!, "Epsilon (положительное) должен возвращать false");
    }

    /// <summary>Проверка максимального значения double</summary>
    [TestMethod]
    public void Convert_MaxValue_ReturnsFalse()
    {
        IValueConverter converter = new IsNegative();
        var result = converter.Convert(double.MaxValue, typeof(bool), null, CultureInfo.InvariantCulture);
        
        Assert.IsFalse((bool)result!, "MaxValue должен возвращать false");
    }

    /// <summary>Проверка минимального значения double</summary>
    [TestMethod]
    public void Convert_MinValue_ReturnsTrue()
    {
        IValueConverter converter = new IsNegative();
        var result = converter.Convert(double.MinValue, typeof(bool), null, CultureInfo.InvariantCulture);
        
        Assert.IsTrue((bool)result!, "MinValue должен возвращать true");
    }
}
