...
namespace MathCore.WPF
{
    public class RadialProgressIndicator : FrameworkElement
    {
        ...
        public static readonly DependencyProperty CurrentValueProperty =
            DependencyProperty.Register(
                ...
                new FrameworkPropertyMetadata(0d,
                    FrameworkPropertyMetadataOptions.AffectsRender,
                    (o, e) => (o as RadialProgressIndicator)?.OnCurrentValueChanged(),
                    (o, e) => DoubleUtil.LessThan((double)e, 0) ? 0 : (DoubleUtil.GreaterThan((double)e, 100) ? 100 : (double)e)));
        ...
        protected override void OnRenderSizeChanged(SizeChangedInfo SizeInfo)
        {
            ...
            var lv_NewRadius = Min(ActualWidth, ActualHeight) / 2;
            var lv_NewCenter = new Point(ActualWidth / 2, ActualHeight / 2);

            if(DoubleUtil.AreClose(_Radius, lv_NewRadius) && DoubleUtil.AreClose(_Center, lv_NewCenter))
                return;

            _Radius = lv_NewRadius;
            _Center = lv_NewCenter;
            CreateGeomerty();
        }
        ...
        private void CreateProgressPath(int variation)
        {
            ...
            {
                var lv_TargetGeometry = (PathGeometry)_CurrentGeometry.Clone();
                lv_TargetGeometry.Transform = new RotateTransform(_RotationAngle, _Center.X, _Center.Y);
                _ProgressGeometry = lv_TargetGeometry.GetFlattenedPathGeometry();
            }

            if(_BorderGeometry != null)
            {
                var lv_TargetGeometry = (PathGeometry)_BorderGeometry.Clone();
                lv_TargetGeometry.Transform = new RotateTransform(-_RotationAngle, _Center.X, _Center.Y);
                _ProgressBorderGeometry = lv_TargetGeometry.GetFlattenedPathGeometry();
            }
        ...
        private void OnUnloaded(object sender, RoutedEventArgs e) { if(_IsListening) StopListening(); }
    ...
    internal static class GeometryExtensions
    {
        ...
        public static double EaseAngle(this double angle)
        {
            ...
            var lv_NormalizedAngle = Abs(angle).Normalize();
            var percentage = lv_NormalizedAngle / 360;

            lv_NormalizedAngle = percentage.EaseInOut(lv_NormalizedAngle, 5, 2);
            lv_NormalizedAngle = Max(lv_NormalizedAngle, 1);

            return sign < 0 ? sign * lv_NormalizedAngle : lv_NormalizedAngle;
        }
        ...
        public static PathGeometry CreatePath(this Point location, double angle, double radius, double InnerRadius)
        {
            ...
            var lv_IsLargeArc = angle > FullCircleInDegrees / 2;

            var lv_ArcPoint = ConvertRadianToCartesian(angle, radius);
            var lv_InnerArcPoint = ConvertRadianToCartesian(angle, InnerRadius);

            var segments = new PathSegmentCollection
            {
                new LineSegment(new Point(location.X, location.Y - radius), false),
                new ArcSegment(new Point(location.X + lv_ArcPoint.X, location.Y + lv_ArcPoint.Y),
                    new Size(radius, radius),
                    0,
                    lv_IsLargeArc,
                    SweepDirection.Clockwise,
                    false),
                new LineSegment(new Point(location.X + lv_InnerArcPoint.X, location.Y + lv_InnerArcPoint.Y), false),
                new ArcSegment(new Point(location.X, location.Y - InnerRadius),
        ...
        /// </returns>
        public static PathGeometry Create(this Point point, int segments, double margin, double radius,
            double InnerRadius)
        {
            if(segments <= 0)
            {
                throw new ArgumentOutOfRangeException($"Segments '{segments}' must be greater than zero.");
            }
            ...
            var lv_AngleSegment = 360d / segments - margin;
            var lv_PathGeometry = new PathGeometry();

            var angle = margin / 2;
            for(var i = 0; i < segments; i++)
            {
                var geometry = point.CreatePath(lv_AngleSegment, radius, InnerRadius);
                geometry.Transform = new RotateTransform(angle, point.X, point.Y);
                var lv_SegmentGeometry = geometry.GetFlattenedPathGeometry();
                lv_PathGeometry.AddGeometry(lv_SegmentGeometry);

                angle += margin + lv_AngleSegment;
            }

            return lv_PathGeometry;
        }
        ...
        public static Point ConvertRadianToCartesian(this double angle, double radius)
        {
            ...
            var lv_AngleRadius = PI / (FullCircleInDegrees / 2) * (angle - FullCircleInDegrees / 4);
            var x = radius * Cos(lv_AngleRadius);
            var y = radius * Sin(lv_AngleRadius);
        ...
        {
            var remainder = angle % FullCircleInDegrees;
    ...
    internal static class DoubleUtil
    {
        ...
        public static bool AreClose(double value1, double value2)
        {
            ...
            var eps = (Abs(value1) + Abs(value2) + 10.0) * DoubleEpsilon;
        ...
        public static bool AreClose(Size size1, Size size2) => AreClose(size1.Width, size2.Width) &&
        ...
        public static bool IsOne(double value) => Abs(value - 1.0) < 10.0 * DoubleEpsilon;
        ...
        public static bool IsZero(double value) => Abs(value) < 10.0 * DoubleEpsilon;
