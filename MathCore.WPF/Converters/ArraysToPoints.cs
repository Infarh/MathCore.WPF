...
namespace MathCore.WPF.Converters
{
    public class ArraysToPoints : MultiValueValueConverter
    {
        protected override object Convert(object[] vv, Type t, object p, CultureInfo c)
        {
            ...
            if(!(vv[0] is IEnumerable)) throw new InvalidCastException("Argument #0 is not IEnumerable<object>");
            if(!(vv[1] is IEnumerable)) throw new InvalidCastException("Argument #1 is not IEnumerable<object>");
            var e1 = (IEnumerable)vv[0];
            var e2 = (IEnumerable)vv[1];
            Func<object, object> f1 = null;
            Func<object, object> f2 = null;
            Point Func(object x, object y) => new Point((double)(f1 ?? (f1 = typeof(double).GetCasterFrom(x.GetType())))(x), (double)(f2 ?? (f2 = typeof(double).GetCasterFrom(y.GetType())))(y));
