...
namespace MathCore.WPF.ViewModels
{
    ...
    public class DynamicViewModel : DynamicObject, INotifyPropertyChanged
    {
        ...
        protected virtual bool SetPropertyValue(string property, object? value)
        {
            switch (property)
            {
                case null: throw new ArgumentNullException(nameof(property));
                case "": throw new ArgumentException("Указанное имя свойства пусто", nameof(property));
            }
        ...
        /// <returns>Истина, если значение было установлено</returns>
        protected virtual bool SetIndexedValue(object[] indexes, object? value)
        {
            switch (indexes)
            {
                case null: throw new ArgumentNullException(nameof(indexes));
                case { Length: 0 } : throw new ArgumentException("Количество индексов должно быть больше 0", nameof(indexes));
            }
        ...
        protected virtual bool TryGetPropertyValue(string property, out object? value) => _PropertiesValues.TryGetValue(property, out value);
        ...
        protected virtual bool TryGetIndexedValue(object[] indexes, out object? value) => _IndexersValues.TryGetValue(indexes, out value);

        /// <inheritdoc />
        public override bool TryGetMember(GetMemberBinder binder, out object? result) => TryGetPropertyValue(binder.Name, out result);

        /// <inheritdoc />
        public override bool TrySetMember(SetMemberBinder binder, object? value) => SetPropertyValue(binder.Name, value);

        /// <inheritdoc />
        public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object? result) => TryGetIndexedValue(indexes, out result);
